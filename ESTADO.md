...
## La hora del recordatorio ahora sí se guarda (2026-09-04)
El usuario preguntó, muy bien visto: cuando en el onboarding se elige una hora para el recordatorio ("8:00 AM/1:00 PM/8:00 PM"), ¿eso queda programado y cómo se le avisa al usuario? Al revisar el código encontré un hueco real: la hora elegida se pedía en pantalla pero **nunca se guardaba en ningún lado** — solo se guardaba un "sí/no quiero recordatorio". Se le explicó con honestidad que además de eso, **no existe ningún mecanismo real de notificaciones push todavía** (nadie recibe avisos aunque active el interruptor) — eso es trabajo aparte que requiere un proveedor externo (ej. Web Push) + un cron que revise horarios, y el usuario decidió NO construir eso todavía. Alcance aprobado: solo arreglar que la hora se guarde de verdad, dejando la infraestructura lista para cuando se construyan las notificaciones reales.

### Implementado
- **`lib/app-state.ts`** (bump v9→v10): campo nuevo `horaRecordatorio: string | null`. `aplicarRecompensaOnboarding` ahora también recibe y guarda la hora elegida en el onboarding (antes se descartaba).
- **`lib/supabase/queries.ts`** + migración `0008_hora_recordatorio.sql` (aplicada en Supabase real): columna `hora_recordatorio text` en `profiles`, incluida en el grant de columnas editables por el usuario.
- **`app/app/perfil/notificaciones/page.tsx`**: cuando el recordatorio está activado, aparece un selector de hora (8:00 AM/1:00 PM/8:00 PM) para que el usuario pueda ver y CAMBIAR la hora después del onboarding, no solo elegirla una vez — coherente con la regla de UX de que todo elemento interactivo debe servir para algo real.
...
## 3 hallazgos reales de una prueba real con un amigo (2026-09-04)
El usuario probó el reto 1v1 con un amigo real y reportó, con evidencia (captura de WhatsApp + observación directa en la app): (1) al amigo que ACEPTA el reto no le aparecía ningún reloj mientras respondía, (2) "Alex y Bruno" (quien solo jugó el reto 1v1, nunca conquistó un país) aparecía primero en el Ranking, por encima de "Dianita" que sí había avanzado con un país, y (3) el progreso de Dianita en Colombia "cambió" — pasó de verse conquistado a mostrar 95%, el sello verde desapareció del Perfil, y Perú ya no redirigía a los planes al tocarlo estando bloqueado.

### Investigación contra la base de datos real
- **Hallazgo 1 confirmado**: `app/app/retos/1v1/[id]/page.tsx` (la pantalla de quien ACEPTA un reto) nunca tuvo cronómetro — a diferencia de `app/app/retos/desafio/page.tsx` (quien CREA el reto), que sí lo tiene y hasta lo manda por WhatsApp. Bug real de paridad, confirmado por código.
- **Hallazgo 2 explicado, no es un bug de por sí**: consultando la base real, NINGÚN usuario tiene todavía un país con `reto_final_completado = true` — ni Dianita ni nadie. El Ranking ordena por países conquistados y, en empate (0 para todos), por nombre alfabético — "Alex" antes que "Dianita" alfabéticamente. Es consecuencia directa del Hallazgo 3, no un bug de ordenamiento.
- **Hallazgo 3 — bug real de fondo, encontrado y corregido**: Dianita completó las 18 rondas de las 6 categorías de Colombia (confirmado en la base: 18/19 etapas = 95%), pero el Reto Final (la 19na etapa, la que de verdad marca el país como conquistado) nunca quedó guardado como completado en el servidor — aunque probablemente localmente sí se veía verde por un momento. Causa raíz real encontrada en `lib/app-state-context.tsx`: el guardado a Supabase corre en un *debounce* de 800ms sin ningún reintento — si el usuario cierra la pestaña/app o pierde conexión en esos 800ms justo después de conquistar un país (muy posible en celular, ej. al salir a compartir el resultado), ese guardado se pierde para siempre y nadie se entera. No hay manera de recuperar el intento anterior de Dianita (no existe historial), pero sí se corrigió para que no le vuelva a pasar a nadie:
  1. **Guardado inmediato para hitos reales**: nueva función `guardarAhora()` en el contexto (`lib/app-state-context.tsx`) que empuja el estado a Supabase EN EL ACTO (sin esperar los 800ms) — usada en `app/app/jugar/[pais]/reto-final/page.tsx` justo al conquistar un país.
  2. **Candado de un solo sentido en la base de datos** (migración `0011_progreso_no_regresa.sql`, aplicada en Supabase real): triggers en `progreso_pais` y `progreso_ronda` que impiden que un guardado con datos viejos "desconquiste" un país o "desapruebe" una ronda ya completada — una vez marcado completado en el servidor, solo puede quedarse igual o mejorar, nunca retroceder. Verificado con una simulación SQL directa (usuario real, fila de prueba): intentar bajar `reto_final_completado`/`completado` de `true` a `false` queda bloqueado por el trigger, confirmado y revertido sin dejar rastro.
  3. **De paso, corregido que `updated_at` de `progreso_pais`/`progreso_ronda` se quedaba congelado** en la fecha del primer guardado (el upsert nunca mandaba esa columna) — esto rompía los filtros Semanal/Mensual del Ranking. Ahora se actualiza en cada guardado real.
- Peru sin redirigir a planes: no encontré ningún cambio de código que explique esto — Perú es gratis (`esGratis: true` desde la Sesión de Rutas Temáticas), así que nunca debería pedir Pro. Es posible que el usuario recuerde un comportamiento de antes de ese cambio. Pendiente de confirmar con más detalle si vuelve a pasar.

### Implementado (código)
- `app/app/retos/1v1/[id]/page.tsx`: cronómetro real (mismo patrón que `desafio/page.tsx`) visible mientras juega, tiempo guardado y mostrado en el resultado junto al de quien creó el reto.
- `app/app/retos/desafio/page.tsx`: ahora guarda su propio tiempo en la fila del reto (antes solo se mostraba en pantalla y en el mensaje de WhatsApp, nunca se guardaba).
- Migración `0010_reto_1v1_tiempo.sql` (aplicada): columnas `tiempo_retador_segundos`/`tiempo_retado_segundos` en `retos_1v1`; `ver_reto_1v1` y `completar_reto_1v1` actualizadas para leer/guardar el tiempo del retado.
- Migración `0011_progreso_no_regresa.sql` (aplicada): triggers anti-retroceso + fix de `updated_at`, descritos arriba.
- `lib/app-state-context.tsx`: nueva función `guardarAhora()`.
...
## Postales + variar formato + souvenirs coleccionables (2026-09-05)
El usuario sintió la app "muy plana, como un examen" — solo preguntas de opción múltiple, sin nada entre medio. Se le dieron 6 ideas (postales, souvenirs, decisiones de ruta, misiones/eventos, ambientación por país, variar formato) y aprobó empezar por 3: postales, mezclar formatos dentro de la ronda, y souvenirs coleccionables. Se le avisó con honestidad de un límite real: no hay fotos de platos/monumentos ni ilustraciones propias todavía, así que "adivinar con imagen" y "Verdadero/Falso" (que necesita redactar cada afirmación a mano) quedaron para una fase 2 — lo de esta sesión usa solo lo que ya existe de verdad (banderas reales, el motor de letras del Ahorcado, datos curiosos verificados).

### Implementado
- **Postales** (`lib/postales-data.ts` + `components/app/Postal.tsx`): 36 datos curiosos reales (6 países × 6 categorías) — se muestra una pantalla con la bandera del país y el dato, SIN preguntas, la primera vez que se entra a una categoría (mientras Explorador de esa categoría siga sin completar). Integrado en `app/app/pais/[pais]/categoria/[categoria]/page.tsx`.
- **Variar formato dentro de la ronda** (`lib/formato-pregunta.ts` + `components/app/CompletarPalabra.tsx`): ~35% de las preguntas elegibles (respuesta sin números, 3-16 letras) se muestran como "completar la palabra" (estilo Ahorcado: teclado A-Z, hasta 5 errores) en vez de opción múltiple — reutiliza la MISMA respuesta que ya tenía la pregunta, sin contenido nuevo. Integrado en la ronda de país (`[ronda]/page.tsx`) y el Reto Final (`reto-final/page.tsx`); las ayudas 50/50 y Pista se deshabilitan en preguntas de este formato (no aplican sin opciones), +10s sigue disponible siempre.
- **Souvenirs coleccionables** (`lib/souvenirs-data.ts` + `components/app/SouvenirVitrina.tsx`): un objeto típico por país (ícono + nombre + dato curioso) para los 6 países con bandera real — se desbloquea en el mismo momento que se conquista el Reto Final (no necesitó campo nuevo en la base de datos: se deriva directo de `progresoPorPais[pais].retoFinalCompletado`, igual que el sello del pasaporte). Vitrina nueva en Perfil, debajo del pasaporte; aviso de "nuevo souvenir" en la pantalla de resultado del Reto Final.
...
## Nuevo reto: "Explora y Descubre" — sopa de letras por país (2026-09-05)
El usuario pidió una sopa de letras temática por país (Colombia, Perú, Chile, Brasil, Cuba, Costa Rica), con 3 niveles de dificultad (Explorador 5 palabras horizontal/vertical 90s, Descubridor 7 palabras +diagonal 80s, Experto 10 palabras +invertidas 80s), que termine con un "¿sabías que?" real del tema. Interacción aprobada: tocar la primera letra + tocar la última letra de la palabra (más fácil en celular que arrastrar). Recompensa aprobada: monto fijo por dificultad (20/30/50 monedas), no por palabra individual.

### Implementado
- **`lib/sopa-letras-generador.ts`** (nuevo): algoritmo de colocación de palabras en una rejilla (más largas primero, hasta 300 intentos por palabra, agranda la rejilla si hace falta), validación de una selección contra la posición real de cada palabra (en cualquiera de los dos sentidos, para que no importe desde qué extremo empiece a tocar). Probado de forma aislada con un script (fuera del proyecto, no quedó en el repo): 360 generaciones (6 temas × 3 niveles × 20 repeticiones), 0 fallos.
- **`lib/sopa-letras-data.ts`** (nuevo): 10 palabras verificadas + su "¿sabías que?" por cada uno de los 6 países (mismos datos ya verificados que las postales). `palabrasPorNivel()` siempre toma las palabras más cortas primero, así Explorador nunca hereda las 10 letras de Experto.
- **3 pantallas nuevas**: `/app/retos/sopa-letras` (elige país) → `/app/retos/sopa-letras/[tema]` (elige dificultad) → `/app/retos/sopa-letras/[tema]/[dificultad]` (el juego real: rejilla + cronómetro + lista de palabras tachadas al encontrarlas + pantalla final con el dato curioso si completó a tiempo).
- Tarjeta nueva "Explora y Descubre" en la pantalla de Retos; ruta de juego agregada a `esRutaDeJuego()` de `BottomNav.tsx` (los menús de tema/dificultad sí mantienen la navegación visible, solo se oculta en la pantalla de juego real).
- **Bug real encontrado y corregido durante la propia verificación**: cuando dos palabras comparten una celda (superposición, normal en cualquier sopa de letras), la celda quedaba `disabled` al encontrarse la primera palabra — bloqueando poder tocarla como extremo de la SEGUNDA palabra que también la necesitaba. Corregido quitando el `disabled`; la celda sigue tocable, solo cambia de color.
...
## Reajuste de economía: Retos = combustible, Ruta = motor lento, Gemas = acelerador (2026-09-08)
El usuario pidió unificar la experiencia: los Retos (minijuegos) pasan a ser la fuente rápida de monedas, la Ruta (Explorador/Descubridor/Experto/Reto Final) se vuelve deliberadamente lenta, y las Gemas sirven para acelerar el avance en la ruta. También pidió que los premios de racha (7/14/30 días) sean la vía real de conseguir Gemas, e incentivar más los Duelos por WhatsApp por ser la puerta de entrada de usuarios nuevos.

### Implementado
- **AppState bump v12→v13** (`lib/app-state.ts`, `conquesta_app_state_v13`): campos nuevos `hitosRachaGanados`, `hitoRachaPendienteDeMostrar`, `tipAcelerarVisto`, `retosGanadosNotificados`. Migración `0014_hitos_racha_y_acelerar.sql` aplicada (columnas + grant de update).
- **Ruta más lenta**: ronda (Explorador/Descubridor/Experto) pagaba `aciertos×5` (hasta ~35-50 monedas) → ahora **5 monedas fijas** por nivel aprobado, 0 si no aprueba (`[ronda]/page.tsx`). Reto Final pagaba `aciertos×5` (hasta 100) → ahora **20 monedas fijas** al conquistar (gemas +3 sin cambio).
- **Retos como combustible**: Ahorcado 15→**25**, Sopa de Letras 30→**40**, Juego de Luces 5→**8**/nivel. Duelos por WhatsApp (`desafio/page.tsx` y `retos/1v1/[id]/page.tsx`) no daban NINGUNA moneda (hueco real encontrado) → ahora **50 monedas al completar** + **15 de bono si le gana a su amigo**, para quien reta y para quien acepta. El bono del retador se paga cuando vuelve a abrir el link y ve que ganó (no lo sabe hasta que el rival juega en otro dispositivo) — idempotente vía `retosGanadosNotificados` (ids de reto ya notificados, para no pagar el bono dos veces si revisita la pantalla).
- **Premios de racha** (`HITOS_RACHA` en `lib/app-state.ts`): día 7 = 50 monedas + 5 gemas, día 14 = 100 + 10, día 30 = 300 + 25 — se otorgan una sola vez por cuenta (se guardan en `hitosRachaGanados`, no se repiten si la racha se rompe y vuelve a llegar al mismo número). Celebración vía `components/app/RachaMilestoneModal.tsx`, montado en `app/app/layout.tsx` (aparece en cualquier pantalla de `/app` en cuanto `registrarActividad` otorga un hito).
- **Gemas como acelerador**: `acelerarPaisConGemas()` + `GEMAS_ACELERAR_PAIS = 20` en `lib/app-state.ts` — completa de un golpe las categorías que falten del país actual + el Reto Final (sin dar monedas, esas se ganan jugando de verdad) y desbloquea el siguiente país de la ruta. UI en `app/app/pais/[pais]/page.tsx`: tarjeta "Acelerar con Gemas" con paso de confirmación (gasto irreversible), deshabilitada si no alcanzan las gemas.
- **Avisos in-app**: tip educativo (cerrable, una sola vez — `tipAcelerarVisto`) en la pantalla del país explicando Retos→monedas→ayudas y Gemas→acelerar. Banner persistente "Gana más monedas en la sección Retos" en la pantalla de categorías del país, con link directo a `/app/retos`.
- ⚠️ Decisión propia (no pedida, avisada aquí): dado que hoy no existe forma de COMPRAR gemas con dinero real, "acelerar con gemas" seguirá siendo un premio raro de usar al principio — si más adelante se quiere vender paquetes de gemas por Hotmart, es un tema aparte (ver `02C-PRICING-Y-MODELO-DE-NEGOCIO.md`).

### Verificación
`npx tsc --noEmit` ✓ · `npm run build` ✓ (24 rutas) · migración aplicada en Supabase (proyecto CONQUESTA, `fxsiwvmchjlyjnsqzevz`) y columnas confirmadas por `information_schema`. Probado en navegador (dev server real, no producción — `AUDIT_BYPASS_AUTH` usado y revertido): tip visible y cerrable en la pantalla de Colombia; "Acelerar con Gemas" mostró el paso de confirmación, y al confirmar con 25 gemas descontó 20 (quedaron 5), las 6 categorías pasaron a 100%, el Reto Final quedó conquistado y la tarjeta de acelerar desapareció (ya no aplica); banner "Gana más monedas en Retos" visible en la pantalla de categoría; modal de racha probado inyectando `hitoRachaPendienteDeMostrar=7` — mostró "+50 monedas +5 gemas" correctamente y se limpió solo al tocar "¡Genial!". Consola sin errores.
⚠️ No se pudo probar de punta a punta el flujo real de streak (jugar 7 días seguidos) ni el bono de victoria del Duelo por WhatsApp con dos sesiones reales distintas (requeriría dos cuentas/navegadores) — la lógica está verificada por lectura de código + `tsc`, pero recomiendo que el usuario confirme jugando un duelo real con un amigo.

## Ranking: 3 pestañas miden cosas distintas de verdad (2026-09-08)
El usuario pidió que el ranking deje de medir siempre "países conquistados" en las 3 pestañas — ahora cada una mide algo distinto: Semanal = monedas ganadas en la semana calendario (domingo a sábado, reinicio real un domingo a la vez, no solo una ventana móvil de 7 días), Mensual = monedas ganadas en el mes calendario, General = países conquistados + insignias ganadas ("Liga de Leyendas").

### Implementado
- **Migración `0015_ranking_por_periodo.sql`**: 4 columnas nuevas en `profiles` (`monedas_semana_base`, `semana_inicio`, `monedas_mes_base`, `mes_inicio`) — nunca escritas por el cliente, solo por un trigger `before update` (`actualizar_bases_ranking`) que en cada guardado revisa si ya cruzamos a una semana/mes nuevo y, si sí, congela la base en el valor de `monedas_ganadas_total` de justo antes de ese guardado. Mismo patrón de "candado de servidor" que ya usan `mejor_nivel_luces`/`monedas_ganadas_total`.
- **RPC reemplazada**: `ranking_paises_conquistados(dias)` → `ranking_jugadores(p_periodo)` (`'semanal'|'mensual'|'general'`). Para semanal/mensual, si la base de ESE jugador no corresponde a la semana/mes actual (no jugó desde que empezó), su valor se lee como 0 en vez de arrastrar monedas de semanas pasadas — así alguien inactivo no queda "flotando" con un número viejo.
- **`app/app/ranking/page.tsx`**: las 3 pestañas ahora muestran el dato correcto por período (monedas ganadas en semanal/mensual, países+insignias en general), con subtítulo explicando qué mide cada una y mensajes de estado vacío distintos.

### Verificación
`npx tsc --noEmit` ✓ · `npm run build` ✓ · migración aplicada en Supabase. Probado con `begin/rollback` en la base real: un `+10` a `monedas_ganadas_total` de un perfil real dejó `semana_inicio` en el domingo correcto (2026-09-06, hoy es martes 2026-09-08) y `monedas_semana_base` congelada en el valor de ANTES del +10 — el cálculo de "monedas ganadas esta semana" da exactamente 10. La función `ranking_jugadores('general')` confirmada devolviendo `países + insignias` como valor. Todo dentro de una transacción con `rollback` — no quedó ningún dato de prueba en la base.
## Cofre semanal: top 3 del Ranking Semanal gana Gemas + medalla exclusiva (2026-09-09)
El usuario aprobó construir el premio pendiente de la sesión anterior. Decisiones técnicas propias (no requerían su aprobación, documentadas aquí): **30/20/10 gemas** para 1º/2º/3º (mismo orden de magnitud que el resto de la economía — el 1er puesto casi cubre solo los 20 gemas de "Acelerar con Gemas"), 3 medallas coleccionables nuevas ("Corona del Campeón Semanal" oro, "Medalla de Plata Semanal", "Medalla de Bronce Semanal" — se ganan una sola vez por cuenta, quedan para siempre en la vitrina), y **America/Bogota (UTC-5 fijo)** como huso horario de referencia para TODO corte de semana/mes de la app (el más representativo de la base de usuarios LATAM).

### Implementado
- **Migración `0016_cofre_semanal.sql`**: activa la extensión `pg_cron` (job scheduler de Postgres, ya disponible en Supabase). Redefine `actualizar_bases_ranking()` y `ranking_jugadores()` para anclar el corte de semana/mes a `America/Bogota` en vez de `current_date` crudo (evita que el trigger y el cron de cierre midan la semana con husos distintos). Nueva función `cerrar_semana_ranking()`: toma el top 3 de quienes tuvieron ganancia positiva en la semana que se acaba de cerrar, reparte gemas + otorga la medalla (idempotente — no la repite si ya la tenían) y marca `medalla_semanal_pendiente` para que el cliente muestre la celebración. Programada con `cron.schedule('cerrar_semana_ranking', '1 5 * * 0', ...)` — domingo 00:01 hora Colombia/Perú.
- **AppState bump v13→v14** (`conquesta_app_state_v14`): `souvenirsSemanalesGanados: string[]` (medallas ganadas, solo lectura desde el cliente — sin permiso de escritura en la base) y `medallaSemanalPendiente: string | null` (sí editable, para que el cliente la limpie a `null` apenas muestra el modal).
- **`components/app/CofreSemanalModal.tsx`** (nuevo, montado en `app/app/layout.tsx`): celebración con confeti al abrir la app y tener una medalla pendiente — mismo patrón que `RachaMilestoneModal`.
- **`components/app/TrofeosSemanales.tsx`** (nuevo, en Perfil debajo de la vitrina de souvenirs): muestra las 3 medallas, bloqueadas ("???") hasta ganarlas de verdad.
- **`lib/medallas-semanales.ts`** (nuevo): datos de las 3 medallas (nombre, puesto, gemas, dato).

### Verificación
`npx tsc --noEmit` ✓ · `npm run build` ✓ · migración aplicada en Supabase, cron confirmado activo (`select * from cron.job` → `1 5 * * 0`, `cerrar_semana_ranking()`). Probé `cerrar_semana_ranking()` completo simulando dos cuentas reales con ganancia de la "semana anterior" (deshabilitando el trigger dentro de una transacción con `rollback`, sin dejar rastro): otorgó correctamente 30 gemas + medalla "oro" al 1º y 20 gemas + "plata" al 2º, y marcó `medalla_semanal_pendiente`. En el navegador (dev server real): con la medalla forzada a "oro" en `localStorage`, el modal del cofre se abrió con el texto y las gemas correctas, se cerró limpiando la bandera, y la vitrina de Perfil pasó de "???" a "Corona del Campeón Semanal" con las otras 2 medallas seguían bloqueadas.
⚠️ Lo único que NO se pudo probar de punta a punta (por ser semanal, no se puede esperar a que llegue el domingo dentro de esta sesión): que el cron REALMENTE dispare solo el domingo a la hora programada. La lógica interna está verificada al 100%; recomiendo revisar el próximo domingo (o consultar `select * from cron.job_run_details order by start_time desc limit 5` en Supabase) que corrió sin errores.

## Pulido de la pantalla de Retos + sopa de letras más grande (2026-09-09)
El usuario pidió: un enunciado llamativo arriba de Retos explicando qué se gana ahí, mover el Reto de Cultura General cerca de las pestañas Tu turno/Esperando/Historial, que el estado vacío no se vea "en blanco o sin gestión", y letras más grandes en la sopa de letras (aunque eso signifique una rejilla un poco más chica).

### Implementado
- **`app/app/retos/page.tsx`**: la tarjeta azul de arriba ahora es un banner con ícono de rayo — "Tu combustible para avanzar: Juega y reta a tus amigos para ganar monedas — te dan ayudas (50/50, +tiempo) y gemas para acelerar tu ruta." El Reto de Cultura General se movió de ser la primera tarjeta a quedar justo ANTES de las pestañas (después de Ahorcado/Luces/Sopa de letras). El estado vacío pasó de una caja con borde punteado (se sentía hueca) a una tarjeta rellena (`bg-surface-secondary`) con título + explicación + un botón real "Retar a un amigo" que lleva al Reto de Cultura General — en "Tu turno" y "Esperando respuesta" (en "Historial" no aplica ningún CTA, solo el mensaje).
- **`lib/sopa-letras-generador.ts`**: la holgura de la rejilla bajó de "+2" a "+1" sobre la palabra más larga (el techo de seguridad de 15 columnas se mantiene igual, así que la generación no se puede romper) — rejillas típicas de 7 palabras ahora salen 1 columna más chicas.
- **`app/app/retos/sopa-letras/page.tsx`**: letras de `text-xs` → `text-sm`, y el ancho máximo de la rejilla de 360px → 380px (más cerca del ancho real de la pantalla).
