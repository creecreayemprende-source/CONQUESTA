...
⚠️ Pendiente real, NO construido en esta sesión (el usuario solo pidió ajustar la MEDICIÓN): el premio de "cofre con Gemas y souvenirs exclusivos" para el top 3 semanal al cerrar la semana. Eso necesita: (a) un cron real (candidato natural: `pg_cron` de Supabase, ya que todo vive ahí) que congele el top-3 cada domingo a medianoche y reparta el premio, y (b) contenido nuevo — qué souvenirs "exclusivos" existen y cuántas gemas por puesto. Avisado al usuario para que decida esos detalles antes de construirlo.
