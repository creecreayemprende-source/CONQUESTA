...
const NIVEL_INFO: Record<NivelSopa, { icon: typeof Compass; desc: string; monedas: number }> = {
  ...
  Experto: { icon: Crown, desc: "Palabras al revés incluidas", monedas: 50 },
};
  ...
  return (
    <div className="flex flex-col px-4">
      <div className="flex items-center gap-2 pt-4">
        ...
        <h1 className="font-display text-lg font-bold text-txt-primary">{pais}</h1>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-3">
        {(Object.keys(SOPA_NIVELES) as NivelSopa[]).map((nivel) => {
          ...
          return (
            <Link
              key={nivel}
              href={`/app/retos/sopa-letras/${encodeURIComponent(pais)}/${nivel}`}
